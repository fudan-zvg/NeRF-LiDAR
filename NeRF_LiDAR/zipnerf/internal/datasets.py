...
from internal import lidar_utils
from internal import obj_utils
from internal import road_augment
import numpy as np
...
class Dataset(torch.utils.data.Dataset):
  ...
  """
    ...
    def __init__(self,
        ...
        self.normal_supervision = config.normal_supervision
        ...
        self.masks = None
        self.intensity = None
        self.bboxes = None # for dynamic objects
        self.timestamps = None
        self.aug_road = config.aug_road # augment road
        self.scale_factor = 1

        self.local2global_idx = None
        self.num_poses = None
        self.num_lidars = 0 if not config.lidar_supervision else 1
        self.lidar_supervision = config.lidar_supervision # False
        self.simulation_mode = config.simulation_mode # default: 'simu', if 'replay', then use the data of GT LiDAR
        self.lidar_batch_ratio = config.lidar_batch_ratio # 0
        self.lidar_depends = None
        ...
        if self.render_path:
            ...
            self.pixtocams = camera_utils.get_pixtocam(self.focal, self.width,
                                                       self.height)
        if split == 'lidar':
            self._n_examples = len(self.lidar_sim[0])
        else:
    ...
    def next_train(self, item):
        if self.lidar_supervision and self.lidar_batch_ratio > 0:
            lidar_batch = self._batch_size//self.lidar_batch_ratio
        else:
            lidar_batch = 0
        if self._patch_size == 1:
            batch =  self._next_train(self._batch_size, self._patch_size)
        else:
            patch_batch = self._batch_size//4
            
            # pix_batch = self._batch_size - patch_batch - lidar_batch
            pix_batch = self._batch_size - patch_batch
            pat = self._next_train(patch_batch, self._patch_size)
            pix = self._next_train(pix_batch, 1)

            if self.aug_road:
                aug_pix = self._augment(pix)

                pix = {k:torch.cat([pix[k],aug_pix[k]],dim=0) if v is not None else None for k,v in pix.items()}
            batch = {}
            ...
            for key in pat.keys():
                if pat[key] is None:
                    ...
                    continue
                # print(f'key is {key}')
                # print(f'dim is {pat[key].shape} for patch, {pix[key].shape} for pix  {lidar[key].shape} for lidar ')
                if pat[key].dim() == 4:
                    batch[key] = torch.cat([pat[key].reshape(-1,pat[key].shape[-1]), pix[key].reshape(-1,pat[key].shape[-1])], 0) 
                elif pat[key].dim() == 3:
                    batch[key] = torch.cat([pat[key].reshape(-1), pix[key].reshape(-1)], 0)
        if self.lidar_supervision:
            lidar = self._next_train(self._batch_size, 1 , lidar_batch= lidar_batch)
        if self.lidar_supervision:

            for key in batch.keys():
                if batch[key] is None or key not in lidar:
                    continue
                # print(f'key is {key}')
                # print(f'batch[key] {batch[key].shape}')
                # print(f'lidar[key] {lidar[key].shape}')
                if batch[key].dim() == 4 or batch[key].dim() == 2:
                    batch[key] = torch.cat([batch[key].reshape(-1,batch[key].shape[-1]), lidar[key]], 0)
                    
                elif batch[key].dim() == 3 or batch[key].dim() == 1:
                    batch[key] = torch.cat([batch[key].reshape(-1), lidar[key]], 0)
    ...
    """
        ...
        if self.masks is not None:
            # batch['img_mask'] = self.masks[cam_idx, pix_y_int, pix_x_int]
            batch['mask'] = self.masks[cam_idx, pix_y_int, pix_x_int]
            # batch['dep_mask'] = self.masks[cam_idx, pix_y_int, pix_x_int]
            # batch['sem_mask'] = self.masks[cam_idx, pix_y_int, pix_x_int]
        if self.local2global_idx is not None:
            batch['glo_idx'] = broadcast_scalar(self.local2global_idx[cam_idx])
        if self.timestamps is not None:
            # current_idx = self.local2global_idx[cam_idx]
            # print(f'current idx is {current_idx}')
           
            
            batch['timestamp'] = broadcast_scalar(self.timestamps[self.local2global_idx[cam_idx]])
            # print(batch['timestamp'])

            batch['bkg'] = batch['mask']==1 # i.e. mask with value 1 is the background part
        ...
        if self.normal_supervision:
            batch['normals'] = self.normal_images[cam_idx, pix_y_int, pix_x_int]
        # if self.lidar_supervision:
        batch['lidar_mask'] = np.zeros_like(batch['mask']) # not lidar
        batch['aug_mask'] = np.zeros_like(batch['mask']) # not augment
        if patch_sample:
            batch['patch_mask'] = np.ones_like(batch['mask']) # not patch
        else:
            batch['patch_mask'] = np.zeros_like(batch['mask']) # not patch

        if self.intensity is not None:
            batch['intensity'] = np.zeros_like(batch['depth'])
        # if self.lidar_supervision and lidar_idx is not None:
        #     # batch_cam = batch.copy()
        #     batch_lidar = self._make_lidar_ray_batch(lidar_idx,lossmult=lossmult)
            # no need to cat
            # for key in batch_cam.keys():
            #     print(f'key is {key}')
            #     if batch_cam[key] is None:
            #         batch[key] = None
            #         continue
            #     print(f'dim is {batch_cam[key].shape} for cam, {batch_lidar[key].shape} for lidar')
            #     if batch_cam[key].shape[-1] == batch_lidar[key].shape[-1]:
            #         batch[key] = np.concatenate([batch_cam[key].reshape(-1,batch_cam[key].shape[-1]),batch_lidar[key]],axis = 0)
            #     else:
            #         batch[key] = np.concatenate([batch_cam[key].reshape(-1),batch_lidar[key]],axis = 0)
        return {k: torch.from_numpy(v.copy()).float() if v is not None else None for k, v in batch.items()}
    def _augment(self,batch_pix,ratio = 4 ):
        sel_n = batch_pix['mask'].shape[0] // ratio
        # only road
        aug_mask = batch_pix['semantic'] == 0
        # batch_aug = {k: v[aug_mask] if v is not None else None for k,v in batch_pix.items()}
        batch_aug = {k:v.clone() if v is not None else None for k,v in batch_pix.items()}
        batch_aug['aug_mask'] = torch.zeros_like(batch_aug['mask'])
        batch_aug['aug_mask'][~aug_mask] = 1 # only on road
        road_n = aug_mask.sum()
        aug_mask = aug_mask.reshape(-1)
        idx = torch.nonzero(aug_mask).squeeze(-1)
        non_idx = torch.nonzero(aug_mask==0).squeeze(-1)
        if road_n>=sel_n:
            sel_idx = idx[:sel_n]
        else:
            # complete
            sel_idx = torch.cat([idx,non_idx[:(sel_n-road_n)]])
        # print('sel_idx shape is ',sel_idx.shape)
        batch_aug = {k:v[sel_idx] if v is not None else None for k,v in batch_aug.items()}
        ...
        batch_aug = road_augment.batch_perturb(batch_aug,delta = 0.1*self.scale_factor)
        #check shape
        
        return  batch_aug
    def _make_lidar_ray_batch(self,
                        # lidar_distances,
                        # lidar_origins,
                        # lidar_directions,
                        lidar_idx,
        ...
        """Creates ray data batch from lidar_origins and lidar_directions
    ...
    Args:
      ...
      cam_idx: int or int array, camera indices. -1 for lidar rays
      lossmult: float array, weight to apply to each ray when computing loss fn.
    ...
    Returns:
      ...
      This is the batch provided for one NeRF train or test iteration.
    """
        _,lidar_distances, lidar_origins,lidar_directions,intensity = self.lidar_depends
        lidar_distances = lidar_distances[lidar_idx]
        lidar_origins = lidar_origins[lidar_idx]
        lidar_directions = lidar_directions[lidar_idx]
        sensor_idx = (self.num_poses + lidar_frame_idx).reshape(-1,1)
        broadcast_scalar = lambda x: np.broadcast_to(x, lidar_distances.shape)
        ray_kwargs = {
            'lossmult': broadcast_scalar(1.) if lossmult is None else lossmult,
            'near': broadcast_scalar(self.near),
            'far': broadcast_scalar(self.far),
        ...
        if self.metadata is not None:
            # Exposure index and relative shutter speed, needed for RawNeRF.
            for key in ['exposure_idx', 'exposure_values']:
                idx = 0 if self.render_path else sensor_idx
                ray_kwargs[key] = broadcast_scalar(self.metadata[key][idx])
        if self.exposures is not None:
            idx = 0 if self.render_path else sensor_idx
            ray_kwargs['exposure_values'] = broadcast_scalar(self.exposures[idx])
        if self.render_path and self.render_exposures is not None:
            ray_kwargs['exposure_values'] = broadcast_scalar(
                self.render_exposures[sensor_idx])

        pixels = dict(**ray_kwargs)

        # # Slow path, do ray computation using numpy (on CPU).
        # batch = camera_utils.cast_ray_batch(self.cameras, pixels, self.camtype)
        batch = lidar_utils.cast_lidar_ray_batch(lidar_origins,
                                                lidar_directions,pixels)

        batch['rgb'] = np.zeros_like(lidar_origins)
        batch['depth'] = lidar_distances.flatten()
        batch['semantic'] = np.ones_like(lidar_distances).flatten()*255
        batch['mask'] = np.ones(lidar_distances.shape[0])
        if self.normal_supervision:
            batch['normals'] = np.repeat(np.array([0,0,1])[None,:],lidar_distances.shape[0],0)
        if self.intensity is not None:
            batch['intensity'] = intensity[lidar_idx].reshape(batch['depth'].shape)
        # if self.local2global_idx is not None:
        #     # batch['glo_idx'] = broadcast_scalar(self.local2global_idx[cam_idx])
        #     batch['glo_idx'] = broadcast_scalar(np.array(cam_idx))
        batch['glo_idx'] = sensor_idx

        batch['lidar_mask'] = np.ones(lidar_distances.shape[0])
        batch['patch_mask'] = np.zeros_like(batch['mask']) # not patch
        if self.timestamps is not None:
            batch['timestamp'] = self.lidar_timestamps[lidar_frame_idx].reshape(-1,1)
        return {k: torch.from_numpy(v.copy()).float() if v is not None else None for k, v in batch.items()}

    def _make_simu_lidar_ray_batch(self,lidar_idx,
                        lossmult = None):
        """Creates ray data batch from lidar_origins and lidar_directions
    ...
    Returns:
      ...
      This is the batch provided for one NeRF train or test iteration.
    """
        lidar_origins_all,lidar_directions = self.lidar_sim
        lidar_origins = lidar_origins_all[lidar_idx]
        if isinstance(lidar_directions,np.ndarray):
            if lidar_origins.shape[0] != lidar_directions.shape[0]:
                lidar_origins = np.broadcast_to(lidar_origins[None,...],lidar_directions.shape)
        else:
            # i.e. directions is list
            lidar_directions = lidar_directions[lidar_idx]
        cam_idx = -1 
        broadcast_scalar = lambda x: np.broadcast_to(x, (lidar_origins.shape[0],1))
        ray_kwargs = {
            'lossmult': broadcast_scalar(1.) if lossmult is None else lossmult,
            'near': broadcast_scalar(self.near),
            'far': broadcast_scalar(self.far),
        ...
        batch['rgb'] = np.zeros_like(lidar_origins)
        # batch['depth'] = lidar_distances.flatten()
        batch['semantic'] = np.ones_like(lidar_origins[:,0]).flatten()*255
        batch['mask'] = np.ones(lidar_origins[:,0].shape[0])
        ...
        if self.timestamps is not None:
            batch['timestamp'] = self.lidar_timestamps[broadcast_scalar(np.array(lidar_idx))].reshape(-1,1)
        return {k: torch.from_numpy(v.copy()).float() if v is not None else None for k, v in batch.items()}
        # return batch
    def _next_train(self, batch_size, patch_size,lidar_batch = 0):
        ...
        if self.lidar_supervision and lidar_batch > 0:
            lidar_frame_idx = np.random.randint(0, len(self.lidar_depends[0]), (lidar_batch,)) # frame idx
            lidar_idx = np.random.randint(0, len(self.lidar_depends[1]), (lidar_batch,)) # all frames idx
        else:
            lidar_idx = None
        ...
        if lidar_idx is None:
            return self._make_ray_batch(pix_x_int, pix_y_int, cam_idx,
                                    lossmult=lossmult,patch_sample = patch_size>1)
        else:
            return self._make_lidar_ray_batch(lidar_idx = lidar_idx,lidar_frame_idx = lidar_frame_idx,
                                    lossmult=lossmult)
    ...
    def generate_ray_batch(self, cam_idx: int):
        ...
        if self.split==utils.DataSplit.LIDAR:
            # at this time, cam_idx is the lidar beam
            return self._make_simu_lidar_ray_batch(cam_idx)
        if self.split==utils.DataSplit.INSTANCE:
            # at this time, cam_idx is the spherical pose idx
...
from .load_nuscenes import load_waymo_meta, load_png_semantic,load_time
from .lidar_utils import load_lidar,get_simu_info,get_gt_info,load_time_lidar
...
class NUSCENES(Dataset):
    def _load_renderings(self, config):
        ...
        if config.forward_facing:
            ...
            # Rotate/scale poses to align ground with xy plane and fit to unit cube.
            poses, transform, scale_factor = camera_utils.transform_poses_pca(poses) # scale=1/max_d
            np.save(os.path.join(root_dir,'c2w_recenter_transform.npy'),transform)
            np.save(os.path.join(root_dir,'scene_scale.npy'),np.array(scale_factor))
            self.near = 2*scale_factor
            self.far = 500*scale_factor
            self.colmap_to_world_transform = transform
            if config.render_spline_keyframes is not None:
        ...
        if config.llff_use_all_images_for_training:
            ...
            train_indices = all_indices % config.llffhold != 0
        video_indice = all_indices[len(all_indices)//6*3:len(all_indices)//6*4] if config.sensor_num == 6 else all_indices
        if config.half_vis:
            video_indice = video_indice[len(video_indice)//2:]

        split_indices = {
            utils.DataSplit.LIDAR: all_indices[:2],
            utils.DataSplit.TEST: all_indices[all_indices % config.llffhold == 0],
            utils.DataSplit.VIDEO: video_indice,
            utils.DataSplit.TRAIN: all_indices[train_indices],
        }
        indices = split_indices[self.split]
        ...
        semantic_names = [img.replace('images', 'labels') for img in image_names]
        mask_names = [img.replace('images', 'mask') for img in image_names]
        # Normals = os.listdir(os.path.join(root_dir,'normals')) 
        normal_names =[os.path.join(root_dir,'normals','{:06d}_normal.png'.format(i)) for i in indices]
        # normal_names = [img.replace('images', 'normal').replace('.png','_normal.png') for img in image_names]



        def load_mask2d(datadir, num_images=250,segmentation = None,indices= None):
            H, W = 900, 1600
            def load_moving_mask(datadir,indices= None,segmentation= None , num_image=num_images):
                ...
                for frame_mask in masks:
                    ...
                    with open(os.path.join(datadir,'mask',frame_mask),'r') as f:
                        ...
                        mask_infos = np.array([mask_info_.split()[-4:] for mask_info_ in mask_infos]).astype(np.int16)
                    ...
                    for mask in mask_infos:
                        ...
                        tmp_label = segmentation[count][mask[0]:mask[2],mask[1]:mask[3]] 
                        tmp_label[~tmp_seg]=255
                        segmentation[count][mask[0]:mask[2],mask[1]:mask[3]] = tmp_label
                    if indices[count] < num:
                        tmp_mask[800:,:] = 0
                    count +=1

                    im_masks.append(tmp_mask)
                return im_masks,segmentation
            img_masks,segmentation = load_moving_mask(datadir,segmentation=segmentation,indices=indices)
        ...
        poses = poses[indices]
        if config.ego_edit:
            poses = obj_utils.edit_poses(poses,shift_dist=config.shift_dist)
        self.pixtocams = self.pixtocams[indices]
        if os.path.exists(os.path.join(root_dir,'timestamps.txt')):
            print('|------load timestamps-----|')
            self.timestamps,time_scale = load_time(root_dir) # require to load timestamp for the frames

            # check camera timestamp aligned
            assert len(self.timestamps) == num_images

            # self.timestamps = np.sort(self.timestamps)
        if self.split == utils.DataSplit.TRAIN:
            # load different training data on different rank
            # local_indices = [i for i in range(len(image_names)) if (i + self.local_rank) % self.world_size == 0]
            # load same training data
            local_indices = [i for i in range(len(image_names))] 
            raw_indices = indices[local_indices]
            

            image_names = [image_names[i] for i in local_indices]
            depth_names = [depth_names[i] for i in local_indices]
            normal_names = [normal_names[i] for i in local_indices]
            semantic_names = [semantic_names[i] for i in local_indices]
            poses = poses[local_indices]
            mask_names = [mask_names[i] for i in local_indices]
            self.pixtocams = self.pixtocams[local_indices]
            ...
            raw_indices = indices
        self.local2global_idx = np.array(indices)
        # self.local2global_idx = np.array(local_indices)
        ...
        if config.rawnerf_mode:
            ...
            # if self.split == utils.DataSplit.TRAIN:
            if self.local_rank==0: print('|------load mask-----|')
            masks,semantics_ = load_mask2d(self.data_dir, num_images=num_images,
                            segmentation=semantics,indices=raw_indices)
            if config.semantic_dilate:
                # update semantic
                semantics = semantics_
            
            if self.local_rank==0: print('|------load lidar-----|')
            recenter_param = [transform, scale_factor]

            if config.instance_obj:
                # sort time 
                timestamps = np.sort(self.timestamps)
                ...
                with open(os.path.join(root_dir,'bboxes.json'),'r') as f:
                    bboxes = json.load(f)
                # bboxes N * 12
                # box.center , box.wlh , np.array((box.orientation[0] , box.orientation[1] , box.orientation[2], box.orientation[3],timestamp, class
                # wlh length, height, width
                track_id = 0
                obj_info = {}
                obj_type_info = {}
                # global coordinates need to be changed into sensor system (for calibration)
                ...
                for instance,annotations in bboxes.items():
                    ...
                    for ann in annotations:
                        ...
                        wlh[1] , wlh [0] =wlh[0] , wlh [1] # make l ,w, h aligned with x y z (front left up)
                        ...
                        # obj_pose[:3,:3] = orient.rotation_matrix
                        # obj_pose[:3,3] = center
                    ...
                    obj_type_info[track_id] = class_type
                    
                    interpolated_pose = obj_utils.pose_interpolation(timestamps,obj_pose)
                    obj_info[track_id] = interpolated_pose # N,9
                    track_id +=1
                # import pdb;pdb.set_trace()
                self.bboxes = (obj_info,obj_type_info)
                tracks_np = []
                for i in range(track_id):
                    tracks_np.append(obj_info[i])
                tracks_np = np.stack(tracks_np)
                np.save(os.path.join(root_dir,'tracks.npy'),tracks_np)


                

            # if self.lidar_supervision and self.split != utils.DataSplit.LIDAR:
            if self.lidar_supervision or self.split == utils.DataSplit.LIDAR:
                i_train_lidar, lidar_distances, lidar_origins,lidar_rays_directions,intensity = \
                    load_lidar(root_dir,moving_mask = not config.instance_obj,recenter_param=recenter_param, )
                self.lidar_depends = [i_train_lidar,lidar_distances, lidar_origins,lidar_rays_directions,intensity]
                
                self.num_lidars = len(i_train_lidar)

                if config.instance_obj:
                    # time_scale is already loaded
                    self.lidar_timestamps = load_time_lidar(os.path.join(root_dir,'lidar_points'),time_scale)

                    assert len(self.lidar_timestamps) == self.num_lidars
                ...
                if config.use_intensity:
                    ...
                    pass
            if self.normal_supervision:
                if self.local_rank==0: print('|------load normal image-----|')
                normals = [utils.load_img(x) for x in tqdm(normal_names)]
                normals_local = np.stack(normals, axis=0).astype(np.float32)[..., :3] / 255. * 2. - 1 
                # poses N * 4 *4 
                # normals N *H *W 3

                # normals = np.einsum('ijk,nhwj->nhwi', poses[:,:3,:3], normals_local)
                normals = (normals_local[:,:,:,None,:] * poses[:,None,None,:3,:3]).sum(axis = -1)
        ...
        if self.split == utils.DataSplit.LIDAR:
            ...
            if self.simulation_mode == 'replay':
                out_name = 'lidar_replay'
                out_dir = os.path.join(config.render_dir, out_name)
                # _, lidar_distances, lidar_origins,lidar_rays_directions = load_lidar(root_dir,recenter_param=recenter_param)
                # # self.lidar_depends = [lidar_distances, lidar_origins,lidar_rays_directions]
                # self.lidar_sim = (lidar_origins,lidar_rays_directions)
                self.lidar_sim = get_gt_info(root_dir,recenter_param=recenter_param,testsavedir=out_dir,frames_num=100)

            else:
                out_name = 'lidar_simulation'
                out_dir = os.path.join(config.render_dir, out_name)
                self.lidar_sim = get_simu_info(root_dir,recenter_param=recenter_param,testsavedir=out_dir)
...
if __name__ == '__main__':
    ...
    config.factor = 8
    # dataset = WAYMO('train', '/SSD_DISK/users/chenyurui/zipnerf/data/waymo/0032150', config)
    dataset = NUSCENES('LIDAR', '/SSD_DISK/users/zhangjunge/nuScenes_scenes/0100', config)
    print(len(dataset))

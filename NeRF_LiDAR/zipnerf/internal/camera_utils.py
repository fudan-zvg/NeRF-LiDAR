...
def transform_poses_pca(poses):
  ...
  """
    ...
    poses_recentered = unpad_poses(transform @ pad_poses(poses)) # forward right down
...
def pixels_to_rays(pix_x_int, pix_y_int, pixtocams,
  ...
  """
    ...
    if pixtocam_ndc is None:
        ...
        pix_x = dx-directions #
        pix_y = dy-directions
        dx_norm = np.linalg.norm(pix_x, axis=-1)
        dy_norm = np.linalg.norm(pix_y, axis=-1)
        pix_base_x = pix_x/np.linalg.norm(pix_x, axis=-1, keepdims=True)
        pix_base_y = pix_y/np.linalg.norm(pix_y, axis=-1, keepdims=True)
    ...
    return origins, directions, viewdirs, radii, imageplane, pix_base_x, pix_base_y


def cast_ray_batch(cameras, pixels, camtype, patch_size=None):
  ...
  """
    ...
    origins, directions, viewdirs, radii, imageplane, base_x, base_y = pixels_to_rays(
        pixels['pix_x_int'],
        pixels['pix_y_int'],

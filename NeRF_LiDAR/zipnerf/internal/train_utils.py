...
def compute_data_loss(batch, renderings, config):
    ...
    for rendering in renderings:
        ...
        if denom > 0:
            ...
            data_losses.append(torch.tensor(0.))
...
def anti_interlevel_loss(ray_history, config):
    ...
    c = last_ray_results['sdist'].detach()
    w = last_ray_results['weights'].detach()
    w_normalize = (w / (c[..., 1:] - c[..., :-1])).clamp_max(10)
    ...
    for i, ray_results in enumerate(ray_history[:-1]):
        ...
        if 'obj_mask' in ray_results:
            loss = ((w_s - wp).clamp_min(0) ** 2 / (wp + 1e-5))[~ray_results['obj_mask']].mean()
            # obj_ray = ray_results['obj_mask'].sum(-1) > 0
            # import pdb;pdb.set_trace()
            # loss += wp[obj_ray][~(ray_results['obj_mask'][obj_ray])].sum()/(obj_ray.sum())
            
        else:
            loss = ((w_s - wp).clamp_min(0) ** 2 / (wp + 1e-5)).mean()
        loss_anti_interlevel += loss
        # cp = ray_results['sdist']
        # wp = ray_results['weights']
        # c_, w_ = stepfun.blur_stepfun(c, w_normalize, config.pulse_width[i])
...
def create_posenet(num_poses, config: configs.Config,num_lidars=0):

    t_ratio = config.t_ratio
    model = posenet_v2.LearnPose(num_poses,num_lidars = num_lidars, t_ratio=t_ratio,learn_R=config.learn_R,learn_t=config.learn_t)
...
def create_tracknet(config: configs.Config,bboxes):
    start_step = config.track_start_opt # 5000
    # t_ratio = config.t_ratio
    model = posenet_v2.Track_opt(bboxes=bboxes,learn_R=True,learn_t=True)
    ...
    lr_kwargs = {
        'max_steps': config.max_steps-start_step,
        'lr_delay_steps': config.lr_delay_steps,
        'lr_delay_mult': config.lr_delay_mult,
    }
    ...
    lr_fn_main = lambda step: math.learning_rate_decay(
        step-start_step,
        lr_init=config.tn_lr_init,
        lr_final=config.tn_lr_final,
        **lr_kwargs)
    optimizer = torch.optim.Adam(model.parameters(), lr=config.tn_lr_init, **adam_kwargs)
...
def edge_aware_loss_v2_norm(rgb, norm,skymask=None,rgb_mask = True,
                       semantic_mask = False,semantic=None, mask=None):
    """Computes the smoothness loss for a disparity image
    The color image is used for edge-aware smoothness
    """
    ...
    grad_disp_x = torch.linalg.norm(norm[:, :, :-1, :] - norm[:, :, 1:, :],dim=-1,keepdim=True)
    grad_disp_y = torch.linalg.norm(norm[:, :-1, :, :] - norm[:, 1:, :, :],dim=-1,keepdim=True)
    if mask is not None:
        mask_x = mask[:,:,:-1]*mask[:,:,1:]
        mask_y = mask[:,:-1,:]*mask[:,1:,:]
        grad_rgb_x = torch.mean(torch.abs(rgb[:, :, :-1, :] - rgb[:, :, 1:, :]), 3, keepdim=True)
...
def latentReg(z, reg):
    return torch.sum(torch.tensor([1 / reg * torch.norm(latent_i) for latent_i in z]))

def create_latent(bboxes,latent_size):
    latent_vector_dict = {}
    obj_info , obj_type_info = bboxes
    initializer = torch.nn.init.normal_

    for obj_idx, obj_info_ in enumerate(obj_info):
         # rename
        # class_name = obj_type_info[obj_idx]
        class_name = 'obj_latent_{}'.format(obj_idx)
        variable = torch.nn.Parameter(initializer(torch.empty(latent_size)), requires_grad=True)
        latent_vector_dict[class_name] = variable

    return latent_vector_dict
...
def volumetric_rendering(rgbs,
  ...
  """
    ...
    #         tdist[..., 0], tdist[..., -1]))
    depth = (weights * t_mids).sum(dim=-1) / acc.clamp_min(eps)
    ...
    if intensity is not None:
        ### no influences to density
        weights_intensity = weights.clone().detach()
        if intensity.shape != weights.shape:
          intensity = intensity.squeeze(-1)
        intensity = (weights_intensity * intensity).sum(dim=-1)
        rendering['intensity'] = intensity

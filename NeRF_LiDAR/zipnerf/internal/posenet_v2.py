...
class Track_opt(nn.Module):
    def __init__(self, bboxes,learn_R=True, learn_t=True, ):
        super(Track_opt, self).__init__()
        self.init_bboxes = bboxes # N_obj, N_timestamp, N_dim_info
        N_obj, N_timestamp,_ = bboxes.shape
        self.opt_r = nn.Parameter(torch.zeros(size=(N_obj,N_timestamp, 1), dtype=torch.float32), requires_grad=learn_R)  # (N, 1)
        self.opt_t = nn.Parameter(torch.zeros(size=(N_obj,N_timestamp, 3), dtype=torch.float32), requires_grad=learn_t)  # (N, 3)
        self.tracks = bboxes

    def forward(self,):

        return self.opt_r,self.opt_t
    
class LearnPose(nn.Module):
    def __init__(self, num_cams,num_lidars=1,learn_R=True, learn_t=True, init_c2w=None, t_ratio=1):
        """
        :param num_cams:
        ...
        self.r = nn.Parameter(torch.zeros(size=(num_cams+num_lidars, 3), dtype=torch.float32), requires_grad=learn_R)  # (N, 3)
        self.t = nn.Parameter(torch.zeros(size=(num_cams+num_lidars, 3), dtype=torch.float32), requires_grad=learn_t)  # (N, 3)
